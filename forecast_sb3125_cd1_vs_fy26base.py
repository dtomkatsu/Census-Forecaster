"""SB 3125 CD1 vs TY2026-frozen baseline — ITEP-comparable distributional analysis.
...
Comparison frame:
  Baseline: Act 46 law frozen at TY2026 (TY2025 brackets + TY2026 standard
            ...
            neutral baseline.
  Bill:     SB 3125 CD1 full implementation (year-specific CD1 brackets +
            Act 46 standard-deduction phase-in continuing as enacted).

Delta = CD1 full - TY2026 frozen law.
...
microsim's income distribution. Results are decomposed into two components:
  bracket_delta: (CD1 brackets + frozen 2026 SDs) - (frozen 2026 brackets + frozen 2026 SDs)
...
Outputs:
  /tmp/cd1_vs_fy26base_bracket_mid_2027_2031.csv
  /tmp/cd1_vs_fy26base_quintile_mid_2027_2031.csv
"""
...
def main() -> None:
    ...
    for yr in TARGET_YEARS:
        ...
        # C: CD1 full (CD1 brackets + Act46 SDs)
        cfg_a = TaxSystemRegistry.get_hb2306_orig_system(yr)   # frozen 2026
        cfg_c = TaxSystemRegistry.get_sb3125_cd1_system(yr)    # CD1 full
        ...
        # TY2026 mortgage/SALT/charitable amounts; CD1 uses target-year scales
        ...
        tax_c = per_unit_tax(proj_target, cfg_c, calc)  # CD1 full (target itemized)

        # Component deltas (filer-level, $)
        bracket_effect = (tax_c - tax_b) * w   # CD1 brackets vs frozen-2026 brackets, holding SD constant
        sd_effect      = (tax_b - tax_a) * w   # Act46 SD expansion effect (ongoing law)
        total_effect   = (tax_c - tax_a) * w   # full delta: CD1 vs frozen-2026
    ...
    Q_CSV = Path("/tmp/cd1_vs_fy26base_quintile_mid_2027_2031.csv")
    B_CSV = Path("/tmp/cd1_vs_fy26base_bracket_mid_2027_2031.csv")
    all_quintiles.to_csv(Q_CSV, index=False)
    all_brackets.to_csv(B_CSV, index=False)

    # ── Print summary ─────────────────────────────────────────────────────────
    print("\n" + "=" * 90, flush=True)
    print("SB 3125 CD1 vs TY2026-FROZEN BASELINE — MID Scenario", flush=True)
