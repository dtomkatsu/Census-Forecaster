"""SB 3125 CD2 fiscal-impact forecast for TY 2027 through 2031.

Compares projected revenue under:
  - Baseline:  Act 46 as currently enacted (Hawaii's 2024 tax-cut phase-in)
  - Scenario:  SB 3125 CD2 (2026 conference draft 2) which overrides §235-51
...
OUT_CSV = Path("/tmp/sb3125_cd2_fiscal_impact_2027_2031.csv")
    ...
    try:
        ...
        for year in TARGET_YEARS:
            ...
            row = {
                ...
                "sb3125_cd2_revenue_$M":    round(scen_row["revenue_millions"], 2),
        ...
        print("\n" + "=" * 110, flush=True)
        print(f"SB 3125 CD2 FISCAL IMPACT ($ millions, vs. Act 46 baseline)", flush=True)
        ...
        if decile_snapshot is not None:
            ...
            decile_snapshot.to_csv("/tmp/sb3125_cd2_decile_TY2027.csv", index=False)
            print("  Saved: /tmp/sb3125_cd2_decile_TY2027.csv", flush=True)
