...
def _apply_wic(units: pd.DataFrame, *, tax_year: int) -> pd.DataFrame:
    """Compute WIC food-package benefit then take-up-impute against USDA-FNS anchor."""
    ...
    try:
        ...
        LOG.warning(
            "WIC admin caseload target unavailable for year=%d (%s); "
            "falling back to TY2022 USDA-FNS anchor.", tax_year, exc,
        )
        target = AdminCaseload.load().target("wic", 2022)
    out = impute_takeup(
        out, target=target, benefit_col="wic_amount",
        score_col="income", ascending=True, weight_col="weight",
    )
    return out
...
def _apply_liheap(units: pd.DataFrame, *, tax_year: int) -> pd.DataFrame:
    """Compute LIHEAP benefit then take-up-impute against HI DHS BESSD anchor."""
    ...
    try:
        ...
        LOG.warning(
            ...
            "falling back to TY2022 HI DHS BESSD anchor.", tax_year, exc,
        )
        target = AdminCaseload.load().target("liheap", 2022)
    out = impute_takeup(
        out, target=target, benefit_col="liheap_amount",
        score_col="income", ascending=True, weight_col="weight",
    )
    return out
...
def _parse_args(argv: Optional[list] = None) -> argparse.Namespace:
    ...
    p.add_argument("--apply-wic", action="store_true", default=True,
                   help="(Default ON.) Impute WIC food-package benefit via "
                        "USDA-FNS-anchored take-up. Use --no-apply-wic to disable.")
    p.add_argument("--no-apply-wic", dest="apply_wic",
                   action="store_false", help=argparse.SUPPRESS)
    p.add_argument("--apply-liheap", action="store_true", default=True,
                   help="(Default ON.) Impute LIHEAP energy-assistance benefit "
                        "via HI DHS BESSD-anchored take-up. Use --no-apply-liheap "
                        "to disable.")
...
def main(argv: Optional[list] = None) -> int:
    ...
    # 6b.i. WIC — USDA-FNS-anchored take-up.
    if args.apply_wic:
        units = _apply_wic(units, tax_year=args.tax_year)

    # 6b.ii. LIHEAP — HI DHS BESSD-anchored take-up.
