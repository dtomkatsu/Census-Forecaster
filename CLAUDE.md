...
- **Never hand-edit COR projections.** `DEFAULT_COR_IIT_PROJECTIONS_M` loads from `census_forecaster/data/cor/cor_iit_projections.json`, refreshed by `python -m census_forecaster.scripts.refresh_cor_iit` and wired into the monthly `refresh-data` workflow. COR meets on no fixed cadence, and the constant silently sat on the March 2026 vintage for months before this existed. To pick up a new meeting, run the script — do not retype the table. The literal in `quintile_analysis.py` is a missing-file fallback only.
